...
Instalación de software:
	\begin{itemize}
	\item OpenCV: Teoricamente este paquete se instala por defecto cuando instalamos \textsf{ros-indigo-desktop-full}. De todas maneras con el siguiente comando podemos asegurarnos de tenerlo perfectamente instalado \textsf{sudo apt-get install ros-indigo-vision-opencv}. Si solo queremos instalar los envoltorios o la posiblidad de usar python tecleamos \textsf{sudo apt-get install python-opnecv}.
	...
	\subsubsection{OpenCV: Leer y mostrar imágenes}	
...
Para poder utilizar estos ejemplos tenemos que asegurarnos de que la Kinect esta funcionando. Las pruebas que yo he realizado es asegurarme de que  freenect esta instalado. 

Lo que tenemos que hacer es conocer si nuestro SO detencta la kinect. solo tenemos que hacer el siguiente comando en un termial 

\begin{listing}[style=consola, numbers=none]
lsusb
\end{listing}

El terminar deberia de darnos un autput como el que se muestra en la siguiente imagen:
...
Lo que he realizado es bajarme el repositorio con el stack de freenect y compilarlo desde mi workspace. A continuacion adjunto la dirección del repositorio \url{https://github.com/ros-drivers/freenect_stack.git}. Solo tendriamos que hacer el git clone en la carpeta donde queremos guardarlo. Antes de arrancar ningun programa de la kinect. Hacemos un lsusb y deberia de aparecer una imagen como la siguiente.
