...
\chapter{Sensores}
En este capitulo comenzamos a trabajar con los sensores, vamos a introducir brevemente los tipo sde sensore que vamos a utilizar. Comenzamos instalando el IDE para programar nuestra placa, a la cual conectaremos nuestros sensores y actuadores. El modelo de la placa que vamos a utilizar lo detallavamos en el apartado 1.2.2 Componentes electronicos.

\section{Placa Controladora embedded}

Las placas controladoras, son normalmente placas de entrada y salida, las cuales pueden enviar señales de control de pulsos digitales a la controladora de los motores y puede recivir señales de entrada desde los sensores. Tambien la podemos utilizar como interfaz para conocer la señales de los encoders de los motores.
Las funciones que va a tener la placa en el robot seran:
\begin{itemize}
\item Hacer de interfaz entre el driver del motor y los encoders.
\item Conectar los sensores de ultrasonidos
\item Enviar y recivir los valores de los sensores al PC y desde el PC.
\end{itemize}

El los siguientes capitulos nos pelearemos conectando a la placa los diferentes elementos. Las placas mas populares en el mercado son los Arduinos, las Raspberry y las Tiva C Launchpad. Nosotros vamos a seleccionar la Tiva C Launchpad por encima del arduino por las siguientes motivos:
\begin{itemize}
\item Tiva C LaunchPad tiene un micro controlador de 32bits ARM Cortex-M4 con 256KB de memoria Flash, 32 KB SRAM y 80Mhz de operacion. Anque la mayoria de arduinos tienen las mismas especificaciones.
\itam %msan TODO%
\end{itemize}
...
Cuando ya tenemos nuestro software instalado y preparado para usarse, vamos a probar que podemos compilar, subir y ejecutar codigo en la placa.
...
Pines & Funcion \\
\hline \hline
Vcc, GND & Estos pines son alimentacion(+) y tierra(-). Normalmente aplicamos 5V.  \\ \hline
