...
\subsection{PCL.}
PCL es un Open Prject para procesado de imagenes 2D/3D y nueves de puntos. El framework PCL tiene incluido numerososo algoritmos de filtrado, segmentacion, reconstruccion y ajuste. Usando estos metodos podemos procesar las Nuves de Puntos y extraer descriptores para reconocer objetos del mundo real basandonos en su geometria   y crear superficies para visualizarlos.

PCL tiene una licencia BSD. Es Open Souerce, gratis para comercio y para uso en investigacion. PCL es multiplataorma y esta perfectamente compilado y depurado en Linux, Mac OS X, Windows y Android. Se puede descargar desde la siguente direccion: http://pointclouds.org/downloads/
...
A continuacion vamos ha mostrar un ejemplo de como podemos integrar todos los puntos anteriores,el software y los frameworks con ROS. Lo primero crearemos un paquete de ROS, con codigo en Python crearemos un programa de Open CV usando el interface con ROS cv\_bridge y usaremos a nuetra kinect para capturar las imagenes gracias al driver de Freenect.

Freenect publicará las imagenes de profundidad y RGB ademas de las capturas de Infrarojos de la cámara.

\subsubsection{Arrancar driver OpenNi/Freenect}
En mi caso OpeNI nunca me funciono para poder visualizar imagenes de la Kinect. La alternativa que encontre fue utilizar el software libre que se llama freenect. En puntos anteriores describo como descargar y arrancar este software para publicar las imagenes en los topicos de ros. Adjunto comandos para arrancar freenect y ver imágenes de la cámara:
...
\subsubsection{Arrancar driver OpenNi/Freenect}
ROS esta compuesto por muchas librerías, OpenCV por su puesto está integrada y su principal uso es para procesar imágenes. El stack de ros se llama vision\_opencv, incluye la libreria completa de OpenCV y la interface para conectarse con ROS.
...
En la siguiente imagen describimos como se conectan los y openCV.

\begin{figure}[h!]
		\caption{Inteface OpenCV-ROS}
		\includegraphics[width=\textwidth]{imagenes/ros-opencv.png}
		\label{fig:5}
		\centering
\end{figure}
